export default function IsaPayTerms() {
  return (
    <div className="prose prose-sm dark:prose-invert max-h-72 overflow-y-auto space-y-3">
      <h3>MyPlug Pay – Terms of Service</h3>
      ...
      <p>Customers have a 72-hour return period starting from the time of delivery confirmation. If a return is initiated within this period: MyPlug Pay will refund the transaction amount to the customer's MyPlug Pay Wallet, not directly to the original payment method. Wallet balances may be used for future purchases or withdrawn (subject to provider fees and processing times). After the 72-hour window expires, transactions are considered final and non-refundable, except where required by law.</p>
      <h4>4. MyPlug Pay Wallet</h4>
      <p>The MyPlug Pay Wallet is a virtual account where refunds and promotional credits are stored. Wallet balances cannot be transferred to another user and may only be withdrawn by the account holder using verified payment methods. MyPlug Pay is not a bank, and wallet balances do not accrue interest.</p>
      <h4>5. Merchant Obligations</h4>
      <p>Merchants must provide accurate item descriptions, delivery timelines, and support for returns. Merchants agree not to request direct payments outside MyPlug Pay. MyPlug Pay reserves the right to suspend or hold payouts if fraudulent activity, disputes, or regulatory concerns arise.</p>
      ...
      <p>We may update these Terms from time to time. Continued use of MyPlug Pay after updates constitutes acceptance of the revised Terms.</p>
      <h4>10. Contact</h4>
      <p>For questions, disputes, or support, please contact MyPlug Technologies Limited at: support@myplug.co.ke</p>
    </div>
  );
}
